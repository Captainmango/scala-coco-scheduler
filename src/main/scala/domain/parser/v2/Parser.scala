        ...
        lexList match {
            ...
            case List(Number(n1), Comma, Number(n2)) => {
                n1.toIntOption match
                    case Some(v1) => n2.toIntOption match {
                        case Some(v2) => Right(CList(List(v1, v2)))
            ...
            case List(Number(n1), Dash, Number(n2)) => {
                n1.toIntOption match
                    case Some(v1) => n2.toIntOption match {
                        case Some(v2) => Right(CRange(bottom = v1, top = v2))
                        case None => Left(UnprocessableNumber(n2))
                    }
                    case None => Left(UnprocessableNumber(n1))
            }
            ...
            case _ => Left(ExpressionInvalid(lexList.foldLeft("") { (acc, l) => 
                acc ++ l.raw
            }))
        }
    }
